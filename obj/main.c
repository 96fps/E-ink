...
int main( int argc, char *argv[] )
{
    ...
    if(argc > 1 && argv[1][0] == 'b'){
         char imageB[20] = "./pic/cB.bmp";
        char imageR[20] = "./pic/cR.bmp";
        if(argc > 4){
            strcpy(imageB, argv[2]);
            strcpy(imageR, argv[3]);
            printf("imageB %s\n", imageB );
            printf("imageR %s\n", imageR );
        }
        ...
        Paint_SelectImage(BlackImage);
        GUI_ReadBmp(imageB, 0, 0);
        printf("read red bmp\r\n");
        Paint_SelectImage(RedImage);
        GUI_ReadBmp(imageR, 0, 0);

        EPD_Display(BlackImage, RedImage);
        DEV_Delay_ms(2000);
    }
    
    if(argc > 1 && argv[1][0] == 'p'){   // show bmp
        printf("Drawing------------------------\r\n");
        Paint_SelectImage(BlackImage);
        Paint_Clear(WHITE);
        ...
        Paint_DrawString_EN(5, 70, "making test", &Font16, WHITE, BLACK);
        ...
        //Paint_DrawString_CN(5, 135,"???bc?ݮ?", &Font12CN, BLACK, WHITE);
